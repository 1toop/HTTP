...
SizeLower=UDim2.new(0,200,1,-56),
UICorner=13,
UIPadding=14,
...
function ak.SetDesc(am,an)
ak.Desc=an
ak.KeybindFrame:SetDesc(an)
end
...
ad.SafeCallback(ak.Callback,ao)
...
local scrollpos = an.UIElements.Menu.Frame.ScrollingFrame.CanvasPosition
...
task.defer(function()
an.UIElements.Menu.Frame.ScrollingFrame.CanvasPosition = scrollpos
end)
...
SideBarWidth=ar.SideBarWidth or 150,
...
task.wait(.45)
if as.Resizable then
if aw:FindFirstChild("ImageLabel") then al(aw.ImageLabel,.45,{ImageTransparency=.8},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play() end
as.CanResize=true
end
end)
...
for _,el in next,as.AllElements do
if el.__type=="Dropdown" and el.Opened and el.Close then
pcall(function() el:Close() end)
end
end
...
if aw:FindFirstChild("ImageLabel") then al(aw.ImageLabel,.3,{ImageTransparency=1},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play() end
j:Set(false)
as.CanResize=false
...
if aw:FindFirstChild("ImageLabel") then al(aw.ImageLabel,0.1,{ImageTransparency=.35}):Play() end
...
if aw:FindFirstChild("ImageLabel") then al(aw.ImageLabel,0.17,{ImageTransparency=.8}):Play() end
end
end)
end
end
